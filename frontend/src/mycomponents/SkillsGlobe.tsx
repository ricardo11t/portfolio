...
const SkillsGlobe: React.FC<SkillGlobeProps> = ({
    ...
    const skillPositions = useMemo(() => {
        ...
        return skills.map((skill, index) => {
            ...
            const x = radius * Math.cos(itemAngle + angle);
            const y = radius * Math.sin(itemAngle + angle);

            const scale = (y+radius) / (2*radius) * 0.75 + 0.25;
        ...
        <div className="relative flex justify-center items-center w-full h-full">
            <div className="relative w-0 h-0 group animate-pulse-slow">
                {skillPositions.map((skill) => (
                    <div
                        key={skill.id}
                        className="absolute top-0 left-0 w-20 h-20 -mt-10 -ml-10 transition-all duration-300 ease-in-out group-hover:[animation-play-state:paused] hover:!scale-125 hover:!z-[101]"
                        style={skill.style}
                    >
